  ...
  return (
    <div className={`relative flex w-full flex-col items-center md:w-1/2 ${alignmentClass}`}>
      <div className={`absolute top-5 hidden md:block ${isLeft ? 'right-[-2px]' : 'left-[-2px]'}`}>
        <div className={`relative h-4 w-4 rounded-full ${isHighlighted ? 'bg-gradient-to-r from-accent to-primary' : 'bg-primary'} shadow-[0_0_10px_hsl(var(--primary))]`}>
          <div className="absolute inset-0 rounded-full bg-primary/30 animate-ping" />
        </div>
      </div>
      <h3 className={`font-headline text-5xl font-bold ${isHighlighted ? 'bg-gradient-to-r from-accent to-primary bg-clip-text text-transparent' : 'text-primary'}`}>{year}</h3>
